...
class DroidNotification:
    ...
    @staticmethod
    def _set_open_behavior(notification, millis):
        '''
        ...
        pending_intent = PendingIntent.getActivity(
            app_context, millis, notification_intent, 0
        )

        notification.setContentIntent(pending_intent)
        notification.setAutoCancel(True)

    def _open_notification(self, notification, millis):
        if SDK_INT >= 16:
            notification = notification.build()
        else:
            notification = notification.getNotification()

        self._get_notification_service().notify(millis, notification)
    ...
    def _notify(self, **kwargs):
        ...
        m1 = int(round(start.timestamp() * 1000))
        m2 = int(round(time.time() * 1000))
        millis = m2 - m1

        self._set_icons(noti, icon=icon)
        self._set_open_behavior(noti, millis)

        # launch
        self._open_notification(noti, millis)